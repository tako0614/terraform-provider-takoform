...
| `Workflow` | `takoform_workflow` | `1.0.0` | Portable durable workflow definition and instance-state lifecycle. |
| `StatefulEntity` | `takoform_stateful_entity` | `1.0.0` | Portable namespace of individually addressable, individually persistent entities. |
| `Schedule` | `takoform_schedule` | `2.0.0` | Portable cron lifecycle that invokes exactly one connected Resource. |

## Data and storage

| Kind | Resource | Version | Portable intent |
| --- | --- | --- | --- |
| `ObjectBucket` | `takoform_object_bucket` | `2.0.0` | Portable object storage with a portable default storage class. |
| `ObjectLifecycleRule` | `takoform_object_lifecycle_rule` | `1.0.0` | Portable retention and transition rule applied to one connected object store. |
| `KeyValueStore` | `takoform_key_value_store` | `1.0.0` | Portable key/value state with an optional consistency preference. |
| `CacheCluster` | `takoform_cache_cluster` | `1.0.0` | Portable in-memory cache sized by an open capability token. |
| `RelationalDatabase` | `takoform_relational_database` | `1.0.0` | Portable relational database addressed through an open engine capability token. |
...
| `Queue` | `takoform_queue` | `2.0.0` | Portable asynchronous delivery with at-least-once semantics. |
...
| `VectorIndex` | `takoform_vector_index` | `2.0.0` | Portable vector index with dimensions fixed for the index lifecycle. |
...
| `ModelEndpoint` | `takoform_model_endpoint` | `1.0.0` | Portable inference endpoint serving one declared model for one declared task. |
...
| `FeatureFlag` | `takoform_feature_flag` | `1.0.0` | Portable named runtime switch with an optional percentage rollout. |
