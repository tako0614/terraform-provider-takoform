...
- `function` (String, required, forces replacement) — Function identity governed by this deployment. There is at most one deployment per identity. Set the name of the target `Function` resource.
- `versions` (List of Object, required) — One or two selected Function Versions and their positive basis-point weights. Weights are host-validated and must sum to exactly 10000. Each entry declares `function_version`, `weight` (between 1 and 10000). The list must declare between 1 and 2 entries.
...
- **Reads dispatch on the recorded FormRef.** `FunctionDeployment` state is addressed under the
...
```console
terraform import takoform_function_deployment.example NAME
terraform import takoform_function_deployment.example SPACE/NAME
```
...
```console
terraform import takoform_function_deployment.example \
  '{"space":"prod","apiVersion":"function.forms.takoform.com","kind":"FunctionDeployment","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
