...
compatibility mapping for the retained Form `vector.forms.takoform.com/VectorIndex`.
...
- `dimension` (Number, required, forces replacement) — Embedding vector dimension fixed for the lifetime of this index. Changing it replaces the index and its records. Between 1 and 1536.
- `metric` (String, required, forces replacement) — Distance metric fixed for the lifetime of this dense index. Changing it replaces the index and its records. One of `cosine`, `euclidean`, `dotproduct`.
...
- **Reads dispatch on the recorded FormRef.** `VectorIndex` state is addressed under the
...
- `vector.index@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_dense_vector_index.example NAME
terraform import takoform_dense_vector_index.example SPACE/NAME
```
...
```console
terraform import takoform_dense_vector_index.example \
  '{"space":"prod","apiVersion":"vector.forms.takoform.com","kind":"VectorIndex","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
