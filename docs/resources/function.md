...
compatibility mapping for the retained Form `function.forms.takoform.com/Function`.
...
- **Reads dispatch on the recorded FormRef.** `Function` state is addressed under the
...
- `function.runtime@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_function.example NAME
terraform import takoform_function.example SPACE/NAME
```
...
```console
terraform import takoform_function.example \
  '{"space":"prod","apiVersion":"function.forms.takoform.com","kind":"Function","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
