...
compatibility mapping for the retained Form `container.forms.takoform.com/ContainerCustomDomain`.
...
- `service` (String, required, forces replacement) — Container Service served on this hostname. Changing it replaces the attachment. Set the name of the target `ContainerService` resource.
- `hostname` (String, required, forces replacement) — Customer-owned DNS hostname served by this attachment. The host canonicalizes it before comparison and storage; ACME certificate issuance is the host's duty.
...
- **Reads dispatch on the recorded FormRef.** `ContainerCustomDomain` state is addressed under the
...
```console
terraform import takoform_container_custom_domain.example NAME
terraform import takoform_container_custom_domain.example SPACE/NAME
```
...
```console
terraform import takoform_container_custom_domain.example \
  '{"space":"prod","apiVersion":"container.forms.takoform.com","kind":"ContainerCustomDomain","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
