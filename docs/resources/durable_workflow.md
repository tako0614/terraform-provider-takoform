...
See the complete exact identity and the [complete example](../../examples/resources/takoform_durable_workflow/resource.tf).
...
- `worker` (String, required, forces replacement) — Module Worker identity whose active deployment serves this workflow class. Changing it replaces the workflow. Set the name of the target `ModuleWorker` resource.
- `class_name` (String, required, forces replacement) — Class export name the serving module provides, in the JavaScript identifier grammar. Every weighted version of the active deployment must export it; one that does not keeps this workflow from becoming Ready, and creating an instance against it is refused rather than queued.
...
- **Reads dispatch on the recorded FormRef.** `DurableWorkflow` state is addressed under the
...
- `worker.workflow@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_durable_workflow.example NAME
terraform import takoform_durable_workflow.example SPACE/NAME
```
