---
page_title: "takoform_actor_namespace Resource - takoform"
subcategory: "tako0614 Forms"
description: |-
  Actor Namespace (edge.forms.takoform.com, role identity).
---
...
This page documents the publisher-set next-major Provider mapping; its exact FormRef `edge.forms.takoform.com/ActorNamespace` is recorded below.
The resource type is Provider metadata. The published Edge Form Definition is maintained in the [takoform-forms source](https://github.com/tako0614/takoform-forms/blob/3231633605b737ce5279d7fc020b4780568e7091/forms/candidates/edge.forms.takoform.com/actor-namespace/definition.json).
See the complete exact identity and the [complete example](https://takoform.com/examples/resources/takoform_actor_namespace/resource.tf).
...
`packageDigest` — Form Package digest (separate from FormRef; embedded Provider provenance): `sha256:eb174b666890ef630e82746374c8a91aec8692c64e98e82dc580a85d17307743`.

## Arguments

- `name` (String, required, forces replacement) — Portable resource name (`metadata.name`).
- `worker` (String, required, forces replacement) — Module Worker identity whose active deployment serves this actor class. Changing it replaces the namespace. Set the name of the target `ModuleWorker` resource.
- `class_name` (String, required, forces replacement) — Class export name the serving module provides, in the JavaScript identifier grammar. Every weighted version of the active deployment must export it; one that does not keeps this namespace from becoming Ready, and invoking a stub of it is refused rather than queued.
...
- **Reads dispatch on the recorded FormRef.** `ActorNamespace` state is addressed under the
...
- `worker.actor@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_actor_namespace.example NAME
terraform import takoform_actor_namespace.example SPACE/NAME
```
...
```console
terraform import takoform_actor_namespace.example \
  '{"space":"prod","apiVersion":"edge.forms.takoform.com","kind":"ActorNamespace","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
