...
| **Legacy lane** | A frozen protocol or Form identity retained for compatibility, recovery, revocation, and migration. The pre-Beta lanes (`forms.takoform.com/v1alpha1` and `/v1alpha2`) were withdrawn ([decision 0042](/spec/decisions/0042-the-pre-beta-epochs-are-withdrawn.html)). |
| **Form Family** | A versionless namespaced Form group ([Form Families](/spec/form-families.html)) such as `edge.forms.takoform.com`: related Forms sharing exact Interface and Binding contracts. The active standalone publisher's Edge candidate set has 16 Experimental `0.x` Forms; Provider 3 retains an eight-family/31-mapping compatibility projection. |
...
| **Lane** | One exact Host protocol path. The public Core checkpoint `1.0.1` uses `forms.takoform.com/v1` independently of Provider publication or Form maturity. Provider `3.0.0` retains the typed eight-family/31-mapping compatibility surface; Provider `2.1.1` remains on retained v1beta1 history. |
