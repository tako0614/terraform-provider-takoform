...
| **Form Family** | A versionless namespaced Form group ([Form Families](/spec/form-families.html)) such as `edge.forms.takoform.com`: related Forms sharing exact Interface and Binding contracts. The current candidate has eight families and 31 Experimental `0.x` FormRefs; Edge contains 16. Family membership and Specification 1.1 grant no Form maturity. |
