...
| **Lane** | A usage path through the single provider address: published `v1.0.3` for Legacy state and recovery; published `v2.0.0` for the nine retained provider-v2 Forms; stable target `v2.1.1` (`candidate-only` until owner publication) for the Experimental Edge Platform Family over the v1beta1 Host API. |
