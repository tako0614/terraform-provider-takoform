...
| **Lane** | A usage path through the single provider address: published `v1.0.3` for Legacy state and recovery; published `v2.0.0` as the compatibility predecessor for the nine retained provider-v2 Forms; Registry-published `v2.1.1` for the Beta Edge Platform Family over the v1beta1 Host API, whose 15 Form definitions are Experimental, while its release descriptor remains `candidate-only` metadata by design after owner publication. |
