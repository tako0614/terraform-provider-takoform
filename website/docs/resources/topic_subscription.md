...
This page documents the current Provider 3 mapping; its current exact FormRef `topic.forms.takoform.com/TopicSubscription` is recorded below.
The resource type is Provider metadata. The exact current mapping is an embedded Provider projection, not a public Form Package: [`forms/candidates/topic.forms.takoform.com/topic-subscription/definition.json`](../../forms/candidates/topic.forms.takoform.com/topic-subscription/definition.json).
See the complete exact identity and the [complete example](https://takoform.com/examples/resources/takoform_topic_subscription/resource.tf).
...
`packageDigest` — Form Package digest (separate from FormRef; embedded Provider provenance): `sha256:c40152191b8013154be18b9599af1990761672617c358ccef25796ac1117afde`.

## Arguments

- `name` (String, required, forces replacement) — Portable resource name (`metadata.name`).
- `topic` (String, required, forces replacement) — Exact Topic identity whose accepted publishes this subscription receives. Changing it replaces the attachment. Set the name of the target `Topic` resource.
- `target` (String, required, forces replacement) — PullQueue resource providing queue.pull@1.0.0. Changing it replaces the attachment. Set the name of the target `PullQueue` resource.
- `filter_policy` (String, optional) — Optional closed attribute-equality filter. When absent, every message matches; each named attribute must equal one of its non-empty candidate strings. There is no range, prefix, negation, or payload-path matching.
- `retry_delay_seconds` (Number, required) — Delay before a failed delivery attempt is retried, in seconds. Between 0 and 3600.
- `max_retries` (Number, required) — Number of delivery retries after the first attempt. A message is attempted at most one plus this value times. Between 0 and 100.
- `dead_letter` (String, optional) — Optional PullQueue receiving messages that exhaust delivery retries. When omitted, exhausted messages are dropped. It must resolve to a different queue from target. Set the name of the target `PullQueue` resource.
...
- **Reads dispatch on the recorded FormRef.** `TopicSubscription` state is addressed under the
...
```console
terraform import takoform_topic_subscription.example NAME
terraform import takoform_topic_subscription.example SPACE/NAME
```
...
```console
terraform import takoform_topic_subscription.example \
  '{"space":"prod","apiVersion":"topic.forms.takoform.com","kind":"TopicSubscription","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
