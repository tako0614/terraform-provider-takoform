...
description: |-
  Static Asset Bundle (edge.forms.takoform.com, role revision).
---
...
This page documents the publisher-set Provider 4 mapping; its exact FormRef `edge.forms.takoform.com/StaticAssetBundle` is recorded below.
The resource type is Provider metadata. The published Edge Form Definition is maintained in the [takoform-forms source](https://github.com/tako0614/takoform-forms/blob/3231633605b737ce5279d7fc020b4780568e7091/forms/candidates/edge.forms.takoform.com/static-asset-bundle/definition.json).
See the complete exact identity and the [complete example](../../examples/resources/takoform_static_asset_bundle/resource.tf).
...
- `manifest_digest` (String, required, forces replacement) — Immutable identity of the committed StaticAssetBundle artifact manifest. Before mutation a host resolves it within the caller's tenant, verifies that its canonical digest matches, that its kind is StaticAssetBundle, and that every file path, media type, size, and blob digest is valid.
...
- **Reads dispatch on the recorded FormRef.** `StaticAssetBundle` state is addressed under the
...
```console
terraform import takoform_static_asset_bundle.example NAME
terraform import takoform_static_asset_bundle.example SPACE/NAME
```
...
terraform import takoform_static_asset_bundle.example \
  '{"space":"prod","apiVersion":"edge.forms.takoform.com","kind":"StaticAssetBundle","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
