...
- `service` (String, required, forces replacement) — Container Service governed by this traffic resource. There is at most one traffic resource per identity. Set the name of the target `ContainerService` resource.
- `revisions` (List of Object, required) — One to eight selected Container Revisions and their positive basis-point weights. Weights are host-validated and must sum to exactly 10000. Each entry declares `container_revision`, `weight` (between 1 and 10000). The list must declare between 1 and 8 entries.
...
- **Reads dispatch on the recorded FormRef.** `ContainerTraffic` state is addressed under the
...
```console
terraform import takoform_container_traffic.example NAME
terraform import takoform_container_traffic.example SPACE/NAME
```
...
```console
terraform import takoform_container_traffic.example \
  '{"space":"prod","apiVersion":"container.forms.takoform.com","kind":"ContainerTraffic","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
