...
compatibility mapping for the retained Form `topic.forms.takoform.com/Topic`.
...
- **Reads dispatch on the recorded FormRef.** `Topic` state is addressed under the
...
- `topic.publish@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_topic.example NAME
terraform import takoform_topic.example SPACE/NAME
```
...
```console
terraform import takoform_topic.example \
  '{"space":"prod","apiVersion":"topic.forms.takoform.com","kind":"Topic","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
