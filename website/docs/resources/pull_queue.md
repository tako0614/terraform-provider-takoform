---
page_title: "takoform_pull_queue Resource - takoform"
subcategory: "Current Form Families"
description: |-
  Pull Queue (queue.forms.takoform.com, role identity).
---
...
compatibility mapping for the retained Form `queue.forms.takoform.com/PullQueue`.
...
- `message_retention_seconds` (Number, required) — Maximum lifetime, in seconds, of an undeleted message. A message older than this bound expires and is removed. Between 60 and 1209600.
- `default_visibility_timeout_seconds` (Number, required) — Default invisibility duration, in seconds, applied by receive when its visibility timeout is omitted. A received message becomes deliverable again after this duration unless deleted or its visibility is changed. Between 0 and 43200.
- `receive_wait_bound_seconds` (Number, required) — Maximum long-poll wait, in seconds, accepted by receive. A receive's waitSeconds is bounded by this value; zero means receive returns without waiting. Between 0 and 20.
- `dead_letter` (Object, optional) — Optional dead-letter policy. Without it, a message that exceeds its receive count is removed only by deletion or retention expiry; when present, the host moves an exhausted message to queue as a new message after maxReceiveCount. The object declares `queue`, `max_receive_count`; when the object is present, every member is required.
...
- **Reads dispatch on the recorded FormRef.** `PullQueue` state is addressed under the
...
- `queue.pull@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_pull_queue.example NAME
terraform import takoform_pull_queue.example SPACE/NAME
```
...
```console
terraform import takoform_pull_queue.example \
  '{"space":"prod","apiVersion":"queue.forms.takoform.com","kind":"PullQueue","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
