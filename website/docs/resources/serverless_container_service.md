...
compatibility mapping for the retained Form `container.forms.takoform.com/ContainerService`.
...
- **Reads dispatch on the recorded FormRef.** `ContainerService` state is addressed under the
...
- `container.runtime@1.0.0` — the exact Interface contract this Form's service exposes.

## Import

```console
terraform import takoform_serverless_container_service.example NAME
terraform import takoform_serverless_container_service.example SPACE/NAME
```
...
```console
terraform import takoform_serverless_container_service.example \
  '{"space":"prod","apiVersion":"container.forms.takoform.com","kind":"ContainerService","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
