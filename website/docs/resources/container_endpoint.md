---
page_title: "takoform_container_endpoint Resource - takoform"
subcategory: "Current Form Families"
description: |-
  Container Endpoint (container.forms.takoform.com, role attachment).
---
...
compatibility mapping for the retained Form `container.forms.takoform.com/ContainerEndpoint`.
...
- `service` (String, required, forces replacement) — Container Service whose active traffic receives HTTPS requests. Changing it replaces the endpoint. Set the name of the target `ContainerService` resource.
...
attribute. Read `takoform_container_endpoint.example.<name>` rather than decoding
...
- **Reads dispatch on the recorded FormRef.** `ContainerEndpoint` state is addressed under the
...
```console
terraform import takoform_container_endpoint.example NAME
terraform import takoform_container_endpoint.example SPACE/NAME
```
...
```console
terraform import takoform_container_endpoint.example \
  '{"space":"prod","apiVersion":"container.forms.takoform.com","kind":"ContainerEndpoint","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
