...
- `function` (String, required, forces replacement) — Function identity this immutable version belongs to. Set the name of the target `Function` resource.
- `artifact` (String, required, forces replacement) — Digest of the committed FunctionBundle artifact manifest. The host resolves and verifies the manifest before storing this revision; a mutable URL or tag is not portable state.
- `handler` (String, required, forces replacement) — Named export of the artifact's main module invoked for each event.
...
- `memory_mib` (Number, required, forces replacement) — Usable memory bound for one invocation environment, in MiB. Between 128 and 10240.
- `timeout_seconds` (Number, required, forces replacement) — Wall-clock invocation budget, in seconds. Expiry terminates the invocation. Between 1 and 900.
- `max_concurrency` (Number, required, forces replacement) — Maximum simultaneous invocations served by one version environment. Between 1 and 1000.
...
- **Reads dispatch on the recorded FormRef.** `FunctionVersion` state is addressed under the
...
```console
terraform import takoform_function_version.example NAME
terraform import takoform_function_version.example SPACE/NAME
```
...
```console
terraform import takoform_function_version.example \
  '{"space":"prod","apiVersion":"function.forms.takoform.com","kind":"FunctionVersion","definitionVersion":"0.1.0","schemaDigest":"sha256:…","name":"…"}'
```
