...
The conformance evidence in this repository is executable, local, and
role-scoped. It proves implementation behavior; it never proves publication,
admission, Host Support, or Form maturity.

- `internal/provider/*_test.go` asserts that the provider built from this
  repository registers exactly the 15 v1beta1 Edge Platform Family resources —
  ...
  and covers typed schema behavior, validation, lane negotiation, and the
  absence of plan-time remote mutation;
- `examples/resources/` contains one formatted HCL example for every
  registered resource, with no exceptions: every provider resource is derived
  from a Form, so every one has an example;
- `cmd/worker-authoring-conformance` drives the raw v1beta1 resources and the
  official `worker-app` module through real OpenTofu and Terraform CLIs
  against the deterministic reference host, as one fail-closed matrix.
...
The provider schema, v1beta1 Form Definitions, and candidate conformance
fixtures are all derived from one family catalog declaration, so drift between
them is a build failure rather than something a checked-in fixture corpus has
to notice later. `go run ./cmd/standard-form-conformance verify` regenerates
nothing: it reads the committed candidate set, re-verifies its bytes, and
inspects the actual provider resource schema for every declared Form,
including that each field the definition calls immutable really forces
replacement.

## Withdrawn corpora

The pre-Beta corpora — `form-package-v1/` (the Legacy v1alpha1 package corpus),
`portable-host-v1/` (the provider-v1 host corpus), `portable-host-v2/` (the
provider-v2 host corpus), and the earlier-withdrawn `portable-host-v3` — are
recorded with the lane each declared in
[`../release/published-document-lanes.json`](../release/published-document-lanes.json),
and their bytes remain in this repository's history
(decisions 0037 and [0042](../spec/decisions/0042-the-pre-beta-epochs-are-withdrawn.md)).
A corpus that cannot resolve what it measures proves nothing by existing, and
a withdrawn corpus address is never reused for a different lane.
