...
export async function runAdmissionSurface({
  ...
  requirePreMutationSourceFence(context, options.expectedCommit);
...
function requireSourceCheckpoint(context, expectedCommit, phase) {
  const dirty = git(context, ["status", "--porcelain"]);
  if (dirty !== "") {
  ...
  if (!CANONICAL_ORIGINS.has(origin)) {
    throw new Error(
      `origin is not the canonical Takoform repository: ${origin}`,
    );
  }
...
function requirePreMutationSourceFence(context, expectedCommit) {
  ...
  if (!CANONICAL_ORIGINS.has(git(context, ["remote", "get-url", "origin"]))) {
    throw new Error(
      "admission publication origin changed after the owner gates",
    );
  }
  const remoteMain = git(context, [
    "ls-remote",
...
function createLocalTag(context, descriptor, expectedCommit, message) {
...
function runCommand(
  ...
  executable,
  args,
  { allowFailure = false, captureOnly = false, preserveOutput = false } = {},
) {
  const environment = { ...process.env };
  delete environment.GH_TOKEN;
  delete environment.GITHUB_TOKEN;
  if (executable === "gh") environment.GH_TOKEN = context.githubToken;
