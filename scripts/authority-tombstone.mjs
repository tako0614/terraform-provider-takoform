...
export function assertAuthorityHistoryContinuity(
  ...
  let activeCommit;
  let activeRecord;
  const parseHistoricalRecord = (record, commit) => {
    let parsed;
    try {
    ...
    try {
      validateAuthorityTombstone(parsed);
    } catch (error) {
      throw new Error(
        `authority tombstone history continuity rejected ${commit}: ${error.message}`,
      );
    }
    return { parsed, record };
  };
  const rememberImmutableActiveRecord = (record, commit) => {
    if (!activeRecord) {
      activeRecord = Buffer.from(record);
      activeCommit = commit;
      return;
    }
    if (!activeRecord.equals(record)) {
      throw new Error(
        `authority tombstone history continuity rejected ${commit}: active record changed after immutable activation ${activeCommit}`,
      );
    }
  };
  for (const commit of chronological) {
    ...
    const { parsed } = parseHistoricalRecord(record, commit);
    ...
    if (status === "active") {
      rememberImmutableActiveRecord(record, commit);
      activeSeen = true;
    }
  }

  // First-parent history catches the ordinary linear revert. Also inspect
  // every record state reachable from HEAD. `--ancestry-path HEAD ^cutoff`
  // is insufficient here because Git can merge an unrelated parent whose
  // own active record must not introduce a second authority history.
  // `--full-history -- <path>` retains every branch on which this record
  // differs, without paying one `git show` for commits that merely inherit
  // the same bytes.
  const ancestryText = runGitText(repositoryRoot, [
    ...
    head,
    "--",
    PREDECESSOR_TOMBSTONE_RECORD_PATH,
  ]);
  ...
  for (const commit of ancestryCommits) {
    ...
    if (record === null) {
      states.set(commit, { status: "missing", record: null });
      continue;
    }
    const { parsed } = parseHistoricalRecord(record, commit);
    ...
    states.set(commit, { status: parsed.status, record });
  }
  ...
  if (currentTombstone?.status === "active" && activeRecord) {
    const workingTreeRecord = readFileSync(
      resolve(repositoryRoot, PREDECESSOR_TOMBSTONE_RECORD_PATH),
    );
    if (!activeRecord.equals(workingTreeRecord)) {
      throw new Error(
        `authority tombstone history continuity rejected working tree: active record changed after immutable activation ${activeCommit}`,
      );
    }
  }
  return {
