...
`resourceVersion`, then sends a generation-fenced observe and maps its
`current` / `drifted` / `missing` result to `drift_status`. The provider does not call the
...
commercial, or executable field vocabulary. The read says what exists and grants nothing; a host MAY filter reads to
already-visible records, but authorization and writes remain host-owned.
...
the exact release-owned identity it names.
