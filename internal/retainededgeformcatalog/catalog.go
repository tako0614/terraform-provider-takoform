// Package retainededgeformcatalog is the FROZEN declaration set of the
// retained edge.forms.takoform.com/v1beta1 generation — the fifteen Form,
// Interface, and Binding contracts exactly as Registry-published provider
// v2.1.1 embedded them. Beta 2 withdrew nothing (decision 0046), so state
// written under these identities must stay decodable for as long as they are
// served; this snapshot is what decodes it, and it never follows the live
// catalog: a released identity means what it meant when it was published
// (decision 0037), so edits here are defects by definition. The provider's
// codec table digest-checks every entry against the identity ledger's released
// schemaDigests, which is what makes the freeze verified rather than asserted.
package retainededgeformcatalog
