...
// RenderedForm is one provider-neutral Form Definition and its derived
// fixtures. ResourceType remains empty in this catalog.
...
func InterfaceRefFor(name, version string) (formpackage.InterfaceRef, error) {
	...
	return formpackage.InterfaceRef{APIVersion: InterfaceAPIVersion, Name: name, Version: version, SchemaDigest: rendered.SchemaDigest}, nil
}
...
func newTargetContractResolver() *targetContractResolver {
	resolver := &targetContractResolver{
		rendered: map[string]RenderedForm{}, refs: map[string]model.TargetFormRef{},
...
func (r *targetContractResolver) TargetFormRefs(targetKind string) ([]model.TargetFormRef, error) {
	...
	if !known {
		return nil, fmt.Errorf("exact-Form target %q is not a Vector catalog Form", targetKind)
	}
	...
	ref := model.TargetFormRef{APIVersion: Family.APIVersion(), Kind: targetKind, DefinitionVersion: form.DefinitionVersion, SchemaDigest: digest}
...
func (r *targetContractResolver) ExactFormRelations(ref model.TargetFormRef) ([]model.Relation, error) {
	...
	if ref.APIVersion != Family.APIVersion() {
		return nil, fmt.Errorf("exact target Form %s is outside Vector catalog group %q", key, Family.APIVersion())
	}
	form, known := ByKind(ref.Kind)
	if !known || form.DefinitionVersion != ref.DefinitionVersion {
		return nil, fmt.Errorf("exact target Form %s is not in this Vector catalog", key)
	}
	...
	if len(refs) != 1 || refs[0] != ref {
		return nil, fmt.Errorf("exact target Form %s does not match rendered Vector identity", key)
	}
...
func (r *targetContractResolver) RequiredInterface(name, version string) (model.RequiredInterface, error) {
	...
	return model.RequiredInterface{APIVersion: ref.APIVersion, Name: ref.Name, Version: ref.Version, SchemaDigest: ref.SchemaDigest}, nil
}
...
func renderConstraints(form model.Form) []formpackage.FormConstraint {
	...
	for _, entry := range declared {
		out = append(out, formpackage.FormConstraint{
			...
			References: append([]string(nil), entry.References...), Anchor: entry.Anchor,
			Members: entry.Members, Through: entry.Through,
		})
	}
	return out
}

// RenderForms renders every Vector Form to its exact Form Definition bytes and
// derived fixtures.
...
func renderForm(form model.Form, resolver model.TargetContractResolver) (RenderedForm, error) {
	...
	for _, item := range negativeCases {
		...
		negative = append(negative, formpackage.NegativeFixture{Name: "reject-" + item.Name, Stage: "desired", InputPath: fixturePath, ExpectedFailure: "schema_validation_failed"})
	}
	...
	definition := formpackage.FormDefinition{
		APIVersion: Family.APIVersion(), Kind: form.Kind, DefinitionVersion: form.DefinitionVersion,
		Title: form.Title, Description: form.Description, Role: string(form.Role),
		RequiresHostAPI: form.RequiresHostAPI, Constraints: renderConstraints(form), DesiredSchema: desiredSchema,
		OutputSchema: outputSchema, ImmutableFields: form.ImmutableFields(), LifecycleCapabilities: form.LifecycleCapabilities(),
