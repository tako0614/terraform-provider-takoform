...
// RenderedForm is one Container Form Definition and its derived fixtures.
// Provider resource mappings are deliberately not part of the source catalog.
...
// It never selects a latest/default Form and never looks outside this family.
...
func (r *TargetResolver) ResolveResourceTarget(target model.ResourceTarget) (model.ResolvedResourceTarget, error) {
	if target.Group != Family.APIVersion() {
		return model.ResolvedResourceTarget{}, fmt.Errorf("container target group %q is outside %q", target.Group, Family.APIVersion())
	}
	if target.Contract.ExactForm && target.Contract.Interface != nil {
		return model.ResolvedResourceTarget{}, fmt.Errorf("container target %s/%s declares both exact Form and Interface contracts", target.Group, target.Kind)
	}
	switch {
	...
	case target.Contract.Interface != nil:
		if target.Kind != "ContainerService" {
			return model.ResolvedResourceTarget{}, fmt.Errorf("container runtime Interface targets must address ContainerService, got %s", target.Kind)
		}
		requested := target.Contract.Interface
		if requested.Name != ContainerRuntimeInterfaceName || requested.Version != interfaceVersion {
			return model.ResolvedResourceTarget{}, fmt.Errorf("container target requires %s@%s, got %s@%s", ContainerRuntimeInterfaceName, interfaceVersion, requested.Name, requested.Version)
		}
	...
	default:
		return model.ResolvedResourceTarget{}, fmt.Errorf("container target %s/%s declares no exact contract", target.Group, target.Kind)
	}
}
...
func (r *TargetResolver) TargetFormRefs(kind string) ([]model.TargetFormRef, error) {
	...
	if r.inProgress[kind] {
		return nil, fmt.Errorf("container exact target cycle through %s", kind)
	}
	form, ok := ByKind(kind)
	if !ok {
		return nil, fmt.Errorf("container exact target %q is not in the catalog", kind)
	}
...
// rendered Definition, not against a default of the same group and kind.
func (r *TargetResolver) ResolveExactFormRelations(ref model.TargetFormRef) ([]model.Relation, error) {
	if ref.APIVersion != Family.APIVersion() {
		return nil, fmt.Errorf("container exact target %s is outside catalog group", ref.String())
	}
	key := ref.String()
	...
	if r.relationsInProgress[key] {
		return nil, fmt.Errorf("container exact relation cycle through %s", key)
	}
	...
	if len(refs) != 1 || refs[0] != ref {
		return nil, fmt.Errorf("container exact target %s does not match rendered catalog identity", ref.String())
	}
	form, ok := ByKind(ref.Kind)
	if !ok || form.DefinitionVersion != ref.DefinitionVersion {
		return nil, fmt.Errorf("container exact target %s is not in this catalog", ref.String())
	}
...
// RequiredInterface resolves one exact digest-bound container.runtime
...
// RenderForms renders every Container Form to its exact definition bytes.
...
// RenderInterfaces renders the exact Container runtime Interface Definition.
...
// SortedKinds is a deterministic source-inspection helper for direct tests.
