...
// RenderedForm is one Pull Queue Form Definition rendered with its canonical
// and negative fixtures. Provider resource mappings are intentionally absent.
...
// TargetResolver resolves exact same-family Form identities and required
// Interface digests for this catalog. No latest/default lookup exists.
...
// NewTargetResolver constructs a fresh exact resolver for this family.
...
// ResolveResourceTarget implements the current model's complete target seam.
// PullQueue references are Group+Kind exact and require the queue.pull
// Interface; a caller cannot silently widen either coordinate.
func (r *TargetResolver) ResolveResourceTarget(target model.ResourceTarget) (model.ResolvedResourceTarget, error) {
	if target.Group != Family.APIVersion() {
		return model.ResolvedResourceTarget{}, fmt.Errorf("queue target group %q is outside %q", target.Group, Family.APIVersion())
	}
	if target.Kind != "PullQueue" {
		return model.ResolvedResourceTarget{}, fmt.Errorf("queue target kind %q is not PullQueue", target.Kind)
	}
	if target.Contract.ExactForm {
		return model.ResolvedResourceTarget{}, fmt.Errorf("queue PullQueue target cannot require an exact self FormRef; require %s@1.0.0", QueuePullInterfaceName)
	}
	if target.Contract.Interface == nil {
		return model.ResolvedResourceTarget{}, fmt.Errorf("queue PullQueue target requires %s@1.0.0", QueuePullInterfaceName)
	}
	if target.Contract.Interface.Name != QueuePullInterfaceName || target.Contract.Interface.Version != "1.0.0" {
		return model.ResolvedResourceTarget{}, fmt.Errorf("queue target requires %s@1.0.0, got %s@%s", QueuePullInterfaceName, target.Contract.Interface.Name, target.Contract.Interface.Version)
	}
	ref, err := InterfaceRefFor(QueuePullInterfaceName, "1.0.0")
	if err != nil {
		return model.ResolvedResourceTarget{}, err
	}
	return model.ResolvedResourceTarget{
		ResourceNamePattern: model.PatternResourceName,
		RequiredInterface: &model.RequiredInterface{
...
// It is useful to a host or family aggregator that must pin a concrete target
// Resource after resolving the queue.pull Interface relation.
func (r *TargetResolver) TargetFormRefs(kind string) ([]model.TargetFormRef, error) {
	...
	if r.inProgress[kind] {
		return nil, fmt.Errorf("queue exact target cycle through %s", kind)
	}
	form, ok := ByKind(kind)
	if !ok {
		return nil, fmt.Errorf("queue exact target %q is not in the catalog", kind)
	}
...
// ResolveExactFormRelations proves the relation contract of one rendered
// concrete queue Form. Interface-open references remain Interface relations;
// a host pins the concrete TargetFormRef of the resolved target Resource at
// admission, after which this method verifies the source-side relation shape.
func (r *TargetResolver) ResolveExactFormRelations(ref model.TargetFormRef) ([]model.Relation, error) {
	if ref.APIVersion != Family.APIVersion() {
		return nil, fmt.Errorf("queue exact target %s is outside catalog group", ref.String())
	}
	key := ref.String()
	...
	if r.relationsInProgress[key] {
		return nil, fmt.Errorf("queue exact relation cycle through %s", key)
	}
	...
	if len(refs) != 1 || refs[0] != ref {
		return nil, fmt.Errorf("queue exact target %s does not match rendered catalog identity", ref.String())
	}
	form, ok := ByKind(ref.Kind)
	if !ok || form.DefinitionVersion != ref.DefinitionVersion {
		return nil, fmt.Errorf("queue exact target %s is not in this catalog", ref.String())
	}
...
// ExactFormRelations is the family-adapter spelling consumed by aggregate
// resolvers.
...
// RequiredInterface resolves the exact digest-bound queue.pull contract.
...
// RenderForms renders every Pull Queue Form to exact Definition bytes.
...
// RenderInterfaces renders every Queue Interface Definition in stable order.
...
// SortedKinds returns stable source-inspection order for tooling and tests.
