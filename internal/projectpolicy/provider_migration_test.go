...
func TestV021ToV1MigrationBoundaryStaysFailClosed(t *testing.T) {
	...
	// Nine retained provider-v2 docs and the eleven Edge Platform Family docs
	// of the Host API v1alpha3 lane. There is no twenty-first document: the
	// generic exact-FormRef carrier was withdrawn (spec/decisions/0021), so
	// every resource document now describes a resource backed by a Form —
	// which is also why the package-digest exemption it needed is gone. Every
	// doc, in either lane, must state the exact Form identity fields the state
	// boundary fences on.
	if len(resourceDocs) != 9+11 {
		t.Fatalf("current resource docs = %d, want %d", len(resourceDocs), 9+11)
	}
