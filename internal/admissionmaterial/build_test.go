...
func TestAdmissionEvidenceWorkflowBindsExactRunsAndSeparatesSignerAuthority(t *testing.T) {
	...
	for _, required := range []string{
		...
		"node scripts/admission-evidence-bindings.mjs",
		"ref: ${{ steps.bindings.outputs.snapshot_commit }}",
		"ref: ${{ github.sha }}",
		"admission_version: ${{ steps.bindings.outputs.admission_version }}",
		"standard-admission-evidence-candidate-${{ needs.assemble.outputs.admission_version }}-",
		"tooling_commit=${GITHUB_SHA}",
