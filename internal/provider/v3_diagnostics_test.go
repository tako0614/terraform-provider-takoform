...
func TestV3DiagnosticRendersEveryRecordedFact(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Form: edge.forms.takoform.com WorkerVersion@0.1.0 schema=sha256:abc",
		"Pointer: /kvBindings/0/resource",
...
func TestV3ParseRelationHostReasonLiftsThePointerAndUIDs(t *testing.T) {
	...
	pointer, expected, current := v3ParseRelationHostReason(
		"relation /kvBindings/0/resource target edge.forms.takoform.com EdgeKVNamespace cache " +
	...
	pointer, expected, current = v3ParseRelationHostReason(
		"relation /worker target edge.forms.takoform.com ModuleWorker counter uid uid-3 no longer exists")
